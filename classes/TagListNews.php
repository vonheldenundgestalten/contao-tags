...
class TagListNews extends TagList
{
	...
	public function getRelatedTagList($for_tags, $blnExcludeUnpublishedItems = true)
	{
		...
		if (count($ids))
		{
			$objTags = $this->Database->prepare("SELECT tag, COUNT(tag) as count FROM tl_tag, tl_news WHERE tl_tag.tid = tl_news.id AND tl_news.pid IN ('" . implode("','", $this->arrNewsarchives) . "') AND from_table = ? AND tl_tag.tid IN (" . implode(",", $ids) . ") " . (!$hasBackendUser ? " AND (start='' OR start<?) AND (stop='' OR stop>?) AND published=1" : "") . " GROUP BY tag ORDER BY tag ASC")
			...
			if ($objTags->numRows)
			{
				while ($objTags->next())
				{
					...
					{
						$count = count($this->Database->prepare("SELECT tl_tag.tid FROM tl_tag, tl_news WHERE tl_tag.tid = tl_news.id AND tl_news.pid IN ('" . implode("','", $this->arrNewsarchives) . "') AND tag = ? AND from_table = ? " . (!$hasBackendUser ? " AND (start='' OR start<?) AND (stop='' OR stop>?) AND published=1" : "") . " AND tl_tag.tid IN (" . implode(",", $ids) . ")")
	...
	public function getTagList($blnExcludeUnpublishedItems = true)
	{
		...
		if (count($this->arrCloudTags) == 0)
		{
			...
			{
				$objTags = $this->Database->prepare("SELECT tag, COUNT(tag) as count FROM tl_tag, tl_news WHERE tl_tag.tid = tl_news.id AND tl_news.pid IN ('" . implode("','", $this->arrNewsarchives) . "') AND from_table = ?" . (!$hasBackendUser ? " AND (start='' OR start<?) AND (stop='' OR stop>?) AND published=1" : "") . " GROUP BY tag ORDER BY tag ASC")
					->execute('tl_news', time(), time());
				$list = "";
				$tags = array();
				if ($objTags->numRows)
